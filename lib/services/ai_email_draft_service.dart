...
class AIEmailDraftService {
  static final AIEmailDraftService _instance = AIEmailDraftService._internal();
  factory AIEmailDraftService() => _instance;
  AIEmailDraftService._internal();
...
''');
  ...
  Future<String> _loadPromptTemplate() async {
    try {
      return await rootBundle.loadString('assets/draft_email.txt');
    } catch (e) {
      print('Error loading email draft prompt template: $e');
      rethrow;
    }
  }
  ...
  Future<String> draftEmailWithOpenAI(String prompt) async {
  ...
  Future<String> draftEmailWithClaude(String prompt) async {
    final apiKeys = await _loadApiKeys();
  ...
  Future<String> draftEmail({
    ...
    try {
      ...
      if (preferredAI == 'claude' && hasClaude) {
        return await draftEmailWithClaude(prompt);
      } else if (preferredAI == 'openai' && hasOpenAI) {
        return await draftEmailWithOpenAI(prompt);
      } else if (hasOpenAI) {
        return await draftEmailWithOpenAI(prompt);
      } else if (hasClaude) {
        return await draftEmailWithClaude(prompt);
